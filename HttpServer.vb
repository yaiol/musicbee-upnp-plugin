...
Partial Public Class Plugin
    Friend Delegate Sub HttpRouteDelegate(request As HttpRequest)
    ...
    Friend Class HttpServer
        ...
        ' How many consecutive ports above the configured one to try before giving up, when the
        ' configured port is unavailable (in use by another app, or in a Windows-reserved range).
        Private Const PortScanRange As Integer = 20

        Public Sub Start()
            Dim configuredPort As Integer = Settings.ServerPort
            Dim boundPort As Integer = 0
            Dim lastEx As Exception = Nothing
            For offset As Integer = 0 To PortScanRange - 1
                Dim tryPort As Integer = configuredPort + offset
                If tryPort > 65535 Then Exit For
                Try
                    listenerSocket = New TcpListener(IPAddress.Any, tryPort)
                    listenerSocket.Start()
                    boundPort = tryPort
                    Exit For
                Catch ex As SocketException
                    ' Port unavailable (EADDRINUSE) or forbidden (EACCES - reserved range). Try the next.
                    lastEx = ex
                    listenerSocket = Nothing
                End Try
            Next offset
            If listenerSocket Is Nothing Then
                LogError(lastEx, "HttpServer.Start")
                Throw lastEx
            End If
            boundServerPort = boundPort
            ' Configured port was taken - we fell back. Tell the user (their saved port won't be
            ' the one in use), and log it. SSDP advertises boundPort, so clients still find us.
            If boundPort <> configuredPort Then
                LogInformation("HttpServer.Start", "configured port " & configuredPort & " unavailable, bound " & boundPort)
                Try
                    System.Windows.Forms.MessageBox.Show(
                        String.Format(Plugin.L("WarnPortInUse"), configuredPort, boundPort),
                        Plugin.L("MessageBoxTitle"),
                        System.Windows.Forms.MessageBoxButtons.OK,
                        System.Windows.Forms.MessageBoxIcon.Warning)
                Catch
                End Try
            End If
            listenerThread = New Thread(New ThreadStart(AddressOf Listen)) With {
                .IsBackground = True,
        ...
        Private Sub ProcessRequest(parameters As Object)
            ...
            Catch ex As SoapException
                LogError(ex, "ProcessRequest:SoapException:" & ex.Code.ToString(), ex.StackTrace)
                Try
                    request.Response.SendSoapErrorHeadersBody(ex.Code, ex.Message)
                Catch
                End Try
